...
const Rent = () => {
    ...
    const addMutation = useMutation((data) => api.create('/units/rent', data), {
        onSuccess: () => {
            ...
            toast({
                ...
                render: () => (
                    <Toast
                        title="Success"
                        description="The rented unit is being processed. "
                    />
                )
            })
        }
    ...
    const onSubmit = async (data) => {
        ...
        if (watch('method') === 'Cash') {
            ...
            addMutation.mutate({
                ...
                price: data.price,
                advance: Number(watch('advance'))
                    ? Number(watch('advance'))
                    : 1,
                deposit: Number(watch('deposit'))
                    ? Number(watch('deposit'))
                    : 1,
    ...
    useEffect(() => {
        if (isUnitFetched) {
            ...
            setValue(
                'total',
                Number(watch('advance')) || Number(watch('deposit'))
                    ? Number(watch('price')) * watch('advance') +
                          watch('price') * watch('deposit')
                    : watch('price')
            )
        }
    }, [isUnitFetched, watch('advance'), watch('deposit')])
